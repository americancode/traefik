...
Purpose: Gateway API stickiness failover when sticky pod is deleted.
...
TEST_DIR="/Users/nathaniel/source/traefik-src/traefik/script/kind-session-persistence/tests/gateway-sticky-failover"
...
COOKIE=$(curl -s -D - -H 'Host: whoami-failover.localhost' http://localhost:8001/ | rg -i '^Set-Cookie:' | head -n1 | sed 's/Set-Cookie: //I' | cut -d';' -f1)
H1=$(curl -s -H 'Host: whoami-failover.localhost' -H "Cookie: ${COOKIE}" http://localhost:8001/ | rg -n '^Hostname:' | awk '{print $2}')
kubectl -n default delete pod -l app=whoami-failover
kubectl -n default rollout status deployment/whoami-failover --timeout=180s
for i in $(seq 1 10); do H2=$(curl -s -H 'Host: whoami-failover.localhost' -H "Cookie: ${COOKIE}" http://localhost:8001/ | rg -n '^Hostname:' | awk '{print $2}'); if [ -n "${H2}" ] && [ "${H1}" != "${H2}" ]; then break; fi; sleep 2; done
echo "h1=${H1} h2=${H2}"
