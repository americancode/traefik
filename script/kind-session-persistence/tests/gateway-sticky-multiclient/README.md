...
Purpose: Gateway API stickiness for two independent clients.
...
TEST_DIR="/Users/nathaniel/source/traefik-src/traefik/script/kind-session-persistence/tests/gateway-sticky-multiclient"
...
COOKIE_A=$(curl -s -D - -H 'Host: whoami-mc.localhost' http://localhost:8001/ | rg -i '^Set-Cookie:' | head -n1 | sed 's/Set-Cookie: //I' | cut -d';' -f1)
COOKIE_B=$(curl -s -D - -H 'Host: whoami-mc.localhost' http://localhost:8001/ | rg -i '^Set-Cookie:' | tail -n1 | sed 's/Set-Cookie: //I' | cut -d';' -f1)
curl -s -H 'Host: whoami-mc.localhost' -H "Cookie: ${COOKIE_A}" http://localhost:8001/ | rg -n '^Hostname:'
curl -s -H 'Host: whoami-mc.localhost' -H "Cookie: ${COOKIE_B}" http://localhost:8001/ | rg -n '^Hostname:'
