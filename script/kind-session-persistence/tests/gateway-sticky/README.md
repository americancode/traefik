...
Purpose: Gateway API sessionPersistence (cookie and header).
...
TEST_DIR="/Users/nathaniel/source/traefik-src/traefik/script/kind-session-persistence/tests/gateway-sticky"
...
COOKIE=$(curl -s -D - -H 'Host: whoami.localhost' http://localhost:8001/ | rg -i '^Set-Cookie:' | head -n1 | sed 's/Set-Cookie: //I' | cut -d';' -f1)
curl -s -H 'Host: whoami.localhost' -H "Cookie: ${COOKIE}" http://localhost:8001/ | rg -n '^Hostname:'
STICKY=$(curl -s -D - -H 'Host: whoami-header.localhost' http://localhost:8001/ | rg -i '^X-Session-Id:' | awk '{print $2}' | tr -d '\r')
curl -s -H 'Host: whoami-header.localhost' -H "X-Session-Id: ${STICKY}" http://localhost:8001/ | rg -n '^Hostname:'
