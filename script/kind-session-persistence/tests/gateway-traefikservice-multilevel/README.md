...
Purpose: TraefikService stickiness at multiple levels.

Hosts:
- whoami-ts-cookie.localhost
- whoami-ts-header.localhost
...
TEST_DIR="/Users/nathaniel/source/traefik-src/traefik/script/kind-session-persistence/tests/gateway-traefikservice-multilevel"
...
RESP=$(curl -s -D - -H 'Host: whoami-ts-cookie.localhost' http://localhost:8001/ -o /dev/null)
...
curl -s -H 'Host: whoami-ts-cookie.localhost' -H "Cookie: ${COOKIE1}; ${COOKIE2}" http://localhost:8001/ | rg -n '^Hostname:'
RESP2=$(curl -s -D - -H 'Host: whoami-ts-header.localhost' http://localhost:8001/ -o /dev/null)
HWR=$(printf '%s' "$RESP2" | rg -i '^X-TS-WRR:' | awk '{print $2}' | tr -d '\r')
HSVC=$(printf '%s' "$RESP2" | rg -i '^X-TS-SVC:' | awk '{print $2}' | tr -d '\r')
curl -s -H 'Host: whoami-ts-header.localhost' -H "X-TS-WRR: ${HWR}" -H "X-TS-SVC: ${HSVC}" http://localhost:8001/ | rg -n '^Hostname:'
