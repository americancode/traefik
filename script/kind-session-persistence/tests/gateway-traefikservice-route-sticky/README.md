...
Purpose: TraefikService sticky should override HTTPRoute sessionPersistence.

Hosts:
- whoami-tsr-cookie.localhost
- whoami-tsr-header.localhost
...
TEST_DIR="/Users/nathaniel/source/traefik-src/traefik/script/kind-session-persistence/tests/gateway-traefikservice-route-sticky"
...
curl -s -D - -H 'Host: whoami-tsr-header.localhost' http://localhost:8001/ -o /dev/null | rg -i '^X-'
curl -s -D - -H 'Host: whoami-tsr-cookie.localhost' http://localhost:8001/ -o /dev/null | rg -i '^Set-Cookie:'
