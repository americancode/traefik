...
Purpose: TraefikService WRR-only sticky should override route stickiness.

Hosts:
- whoami-tsr-wrr-cookie.localhost
- whoami-tsr-wrr-header.localhost
...
TEST_DIR="/Users/nathaniel/source/traefik-src/traefik/script/kind-session-persistence/tests/gateway-traefikservice-route-wrr"
...
curl -s -D - -H 'Host: whoami-tsr-wrr-header.localhost' http://localhost:8001/ -o /dev/null | rg -i '^X-'
curl -s -D - -H 'Host: whoami-tsr-wrr-cookie.localhost' http://localhost:8001/ -o /dev/null | rg -i '^Set-Cookie:'
