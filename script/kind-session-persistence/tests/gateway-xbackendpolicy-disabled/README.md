...
Purpose: XBackendTrafficPolicy ignored when experimentalChannel is false.
...
TEST_DIR="/Users/nathaniel/source/traefik-src/traefik/script/kind-session-persistence/tests/gateway-xbackendpolicy-disabled"
...
curl -s -D - -H 'Host: whoami-xbtp-disabled.localhost' http://localhost:8001/ -o /dev/null | rg -i '^X-Policy-Session:' || true
for i in $(seq 1 10); do curl -s -H 'Host: whoami-xbtp-disabled.localhost' http://localhost:8001/ | rg -n '^Hostname:'; done | sort -u
