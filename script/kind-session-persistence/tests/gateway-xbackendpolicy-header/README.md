...
Purpose: XBackendTrafficPolicy header stickiness for a Service.
...
TEST_DIR="/Users/nathaniel/source/traefik-src/traefik/script/kind-session-persistence/tests/gateway-xbackendpolicy-header"
...
POLICY=$(curl -s -D - -H 'Host: whoami-xbtp.localhost' http://localhost:8001/ -o /dev/null | rg -i '^X-Policy-Session:' | awk '{print $2}' | tr -d '\r')
curl -s -H 'Host: whoami-xbtp.localhost' -H "X-Policy-Session: ${POLICY}" http://localhost:8001/ | rg -n '^Hostname:'
