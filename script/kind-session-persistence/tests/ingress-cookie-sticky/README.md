...
Purpose: Kubernetes Ingress service annotations for cookie-based stickiness.

Host:
- whoami-ingress-cookie.localhost
...
TEST_DIR="/Users/nathaniel/source/traefik-src/traefik/script/kind-session-persistence/tests/ingress-cookie-sticky"
...
COOKIE=$(curl -s -D - -H 'Host: whoami-ingress-cookie.localhost' http://localhost:8001/ | rg -i '^Set-Cookie:' | rg 'ingress-sticky' | head -n1 | sed 's/Set-Cookie: //I' | cut -d';' -f1)
curl -s -H 'Host: whoami-ingress-cookie.localhost' -H "Cookie: ${COOKIE}" http://localhost:8001/ | rg -n '^Hostname:'
