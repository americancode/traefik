...
Purpose: IngressRoute stickiness (cookie + header).

Hosts:
- whoami-ir-cookie.localhost
- whoami-ir-header.localhost
...
TEST_DIR="/Users/nathaniel/source/traefik-src/traefik/script/kind-session-persistence/tests/ingressroute-sticky"
...
COOKIE=$(curl -s -D - -H 'Host: whoami-ir-cookie.localhost' http://localhost:8001/ | rg -i '^Set-Cookie:' | head -n1 | sed 's/Set-Cookie: //I' | cut -d';' -f1)
curl -s -H 'Host: whoami-ir-cookie.localhost' -H "Cookie: ${COOKIE}" http://localhost:8001/ | rg -n '^Hostname:'
STICKY=$(curl -s -D - -H 'Host: whoami-ir-header.localhost' http://localhost:8001/ | rg -i '^X-IR-Sticky:' | awk '{print $2}' | tr -d '\r')
curl -s -H 'Host: whoami-ir-header.localhost' -H "X-IR-Sticky: ${STICKY}" http://localhost:8001/ | rg -n '^Hostname:'
